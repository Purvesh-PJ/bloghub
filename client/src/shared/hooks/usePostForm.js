...
const usePostForm = (isEditing, postId) => {
  // HOOKS
  const { fetchPost, postData, postCategories, postLoading, postError } = useFetchSinglePost(
    postId,
    false,
  );
  ...
  const handlePostRemoveCategory = (removedCategoryIndex) => {
    ...
    const updateCategories = selectedCategories.filter(
      (_, index) => index !== removedCategoryIndex,
    );
    ...
    if (
      isEditing &&
      postAttachedCategories.includes(categoryToRemove) &&
      !removedCategories.includes(categoryToRemove)
    ) {
