...
const useCategoryActions = () => {
  ...
  const addNewCategoriesToPost = useCallback(async (categories, postId) => {
    ...
    try {
      ...
      setAddNewCategoriesError(
        `Error posting categories: ${error.response?.status || error.message}`,
      );
      return false;
    } finally {
  ...
  const updateCategoriesOfPost = useCallback(
    async (categoriesToAdd, categoriesToRemove, postId) => {
      setCategoriesUpdationLoading(true);
      try {
        const response = await updateCategoryCollection(
          categoriesToAdd,
          categoriesToRemove,
          postId,
        );
        ...
        setCategoriesUpdationError(
          `Error updating categories: ${error.response?.status || error.message}`,
        );
        return false;
      } finally {
