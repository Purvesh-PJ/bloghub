...
import {
  ...
  ColorPicker,
  ColorOption,
} from './Page_AdminCategories.styles';

import { Plus, Pencil, Trash2, Tags, FolderPlus } from 'lucide-react';
...
const Page_AdminCategories = () => {
  ...
  const handleUpdateItem = () => {
    ...
    if (activeTab === 'categories') {
      ...
      setTags((prev) =>
        prev.map((item) => (item.id === selectedItem.id ? { ...item, name: formData.name } : item)),
      );
    }
  ...
  return (
    <Container>
      ...
      <ContentContainer>
        <LeftPanel>
          <PanelHeader>
            <PanelTitle>{activeTab === 'categories' ? 'All Categories' : 'All Tags'}</PanelTitle>
          </PanelHeader>
          <PanelContent>
            <CategoryList>
              ...
              {(activeTab === 'categories' ? categories.length === 0 : tags.length === 0) && (
                <EmptyState>
                  <EmptyState.Icon $size="60px">
                    {activeTab === 'categories' ? <FolderPlus /> : <Tags />}
                  </EmptyState.Icon>
                  <EmptyState.Title>
                    No {activeTab === 'categories' ? 'categories' : 'tags'} found
                  </EmptyState.Title>
                  <EmptyState.Description>
                    Create your first {activeTab === 'categories' ? 'category' : 'tag'} to organize
                    your content
                  </EmptyState.Description>
                  <Button
                    className="primary"
