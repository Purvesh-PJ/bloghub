...
import PostAuthor from '../FullpostComponents/PostAuthor';
...
export const Post = ({ blogs }) => {
  ...
  const replaceEmptyParagraphs = (domNode) => {
    if (domNode.type === 'tag' && domNode.name === 'p') {
      const hasOnlyWhitespace = (domNode.children || []).every(
        (child) =>
    ...
    <Container>
      <Article>
        <ArticleBody>
          ...
          <CoverImage>
            <PostImage src={blogs?.imageURL} alt={blogs?.title || 'Post image'} />
          </CoverImage>
