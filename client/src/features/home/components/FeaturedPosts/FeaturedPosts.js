...
import { SectionHeader } from '../../pages/Main/Page_Home.styles';
    ...
    <FeaturedContainer>
      ...
      {posts.length > 0 ? (
        <FeaturedGrid>
          ...
          <MainFeature>
            <MainFeatureImage>
              <img src={mainPost.imageURL || defaultImage} alt={mainPost.title} />
            </MainFeatureImage>

            <MainFeatureContent>
              <MainFeatureCategory>{mainPost.category?.name || 'Featured'}</MainFeatureCategory>
            ...
            {secondaryPosts.map((post) => (
              <SecondaryPost key={post._id}>
                <SecondaryPostImageContainer>
                  <SecondaryPostCategory>{post.category?.name || 'General'}</SecondaryPostCategory>
        ...
        <EmptyMessage>No featured posts available. Check back soon for new content!</EmptyMessage>
      )}
    </FeaturedContainer>
  );
};
