...
import { FaUser, FaChartBar, FaTag, FaEnvelope, FaMapMarkerAlt, FaGlobe, FaCalendarAlt, 
  FaUserTag, FaFileAlt, FaUsers, FaUserFriends, FaThumbsUp, FaPen, FaComment, FaHeart } from 'react-icons/fa';
import {
  ProfilePage,
  ProfileCard,
  ProfileHeader,
...
const Profile = () => {
  ...
  if (isLoading || !user) {
    return (
      <ProfilePage>
        <ProfileCard>
          <LoadingContainer>Loading profile data...</LoadingContainer>
        </ProfileCard>
      </ProfilePage>
    );
  }
  ...
  const userInterests = user?.profile?.interests || ['Technology', 'Writing', 'Web Development', 'Travel', 'Photography'];
