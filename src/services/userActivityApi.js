...
export const getUserActivity = async (userId, page = 1, limit = 10) => {
  const response = await api.get(`${API_BASE_URL}/user-activity/user/${userId}?page=${page}&limit=${limit}`, {
    headers: {
      'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
    }
  });
...
export const getModerationLog = async (page = 1, limit = 20) => {
  const response = await api.get(`${API_BASE_URL}/user-activity/moderation-log?page=${page}&limit=${limit}`, {
    headers: {
      'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
    }
  });
