...
import { 
  ...
  LegendColor,
  LegendLabel
} from './Page_UserAnalytics-Style';
import { LineChart, Line, AreaChart, Area, BarChart, Bar, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';
import { FaThumbsUp, FaFileAlt, FaChartLine, FaArrowUp, FaUsers, FaFilter, FaSortAmountDown, FaSortAmountUp } from 'react-icons/fa';
import { Eye, FileText, Users, MessagesSquare  } from 'lucide-react';
import ReusableTable from '../../../Components/UI-Components/Table/Table';
...
const Page_UserAnalytics = () => {
  ...
  const filteredPosts = topPosts
    .filter(post => categoryFilter === 'all' || post.category.toLowerCase() === categoryFilter.toLowerCase())
    ...
    <Container>
        ...
        <>
          <OverviewSection>
            <OverviewGrid>
              <StatCard>
                ...
                <Eye size={20} strokeWidth={2} color="oklch(37.3% 0.034 259.733)" />
                </StatIcon>
                <StatContent>
              ...
              <StatCard>
                ...
                <FileText size={20} strokeWidth={2} color="oklch(37.3% 0.034 259.733)" />
                </StatIcon>
                <StatContent>
              ...
              <StatCard>
                <StatIcon color="#f59e0b">
                  <Users size={20} strokeWidth={2} color="oklch(37.3% 0.034 259.733)" />
                </StatIcon>
                <StatContent>
              ...
              <StatCard>
                <StatIcon color="#ef4444">
                  <MessagesSquare size={20} strokeWidth={2} color="oklch(37.3% 0.034 259.733)" />
                </StatIcon>
                <StatContent>
          ...
          <ChartSection>
            <ChartContainer>
              <ChartHeader>
                <ChartTitle>
                  <FaChartLine color="oklch(0.446 0.03 256.802)" /> Blog Views
                </ChartTitle>
              </ChartHeader>
                ...
                {/* <AreaChart data={monthlyViewsData}>
                  <defs>
                    <linearGradient id="viewsGradient" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="oklch(78.9% 0.154 211.53)" stopOpacity={0.1}/>
                      <stop offset="95%" stopColor="oklch(78.9% 0.154 211.53)" stopOpacity={0.1}/>
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="1 1" stroke="oklch(55.4% 0.046 257.417)" />
                  <XAxis dataKey="name" />
                  <YAxis />
                  <Tooltip />
                  <Area type="monotone" dataKey="views" stroke="oklch(55.4% 0.046 257.417)" fillOpacity={1} fill="url(#viewsGradient)" />
                </AreaChart> */}
            ...
            <ChartContainer>
              <ChartHeader>
                <ChartTitle>
                  <FaUsers color="oklch(0.446 0.03 256.802)" /> Traffic Sources
                </ChartTitle>
              </ChartHeader>
                ...
                <PieChartLegend>
                  {trafficSourcesData.map((item, index) => (
                    <LegendItem key={index}>
                      ...
                      <LegendLabel>{item.name} ({item.value}%)</LegendLabel>
                    </LegendItem>
                  ))}
                </PieChartLegend>
              </div>
          ...
          <ChartSection>
            <ChartContainer>
              <ChartHeader>
                <ChartTitle>
                  <FaThumbsUp color="oklch(0.446 0.03 256.802)" /> Category Engagement
                </ChartTitle>
              </ChartHeader>
              <ResponsiveContainer width="100%" height={300}>
                ...
                <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.3} />
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="likes" name="Likes" fill="#f59e0b" radius={[4, 4, 0, 0]} barSize={20} />
                  <Bar dataKey="comments" name="Comments" fill="#ef4444" radius={[4, 4, 0, 0]} barSize={20}/>
                  <Bar dataKey="shares" name="Shares" fill="#10b981" radius={[4, 4, 0, 0]} barSize={20}/>
                </BarChart>
              </ResponsiveContainer>
            </ChartContainer>
            ...
            <ChartContainer>
              <ChartHeader>
                <ChartTitle>
                  <FaUsers color="oklch(0.446 0.03 256.802)" /> Subscriber Growth
                </ChartTitle>
              </ChartHeader>
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={subscriberGrowthData}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Area type="monotone" dataKey="subscribers" stroke="#10b981" fillOpacity={1} fill="url(#subscriberGradient)" />
                </AreaChart>
              </ResponsiveContainer>
            </ChartContainer>
          </ChartSection>
      ...
      {activeTab === 'posts' && (
        <>
          <FilterControls>
            <FilterGroup>
              <FilterLabel>
                <FaFilter color='oklch(0.551 0.027 264.364)' /> Filter by:
              </FilterLabel>
              ...
              <DropdownFilter value={authorFilter} onChange={handleAuthorFilterChange}>
                {authors.map((author, index) => (
                  <option key={index} value={author === 'All' ? 'all' : author}>{author}</option>
                ))}
              </DropdownFilter>
            </FilterGroup>
          </FilterControls>
          ...
          <ChartContainer>
            <ChartHeader>
              <ChartTitle>
                <FaFileAlt color='oklch(0.551 0.027 264.364)' /> Top Performing Posts
              </ChartTitle>
            </ChartHeader>
            <ReusableTable 
              data={filteredPosts} 
      ...
      {activeTab === 'authors' && (
        <>
          <ChartContainer>
            <ChartHeader>
              <ChartTitle>
                <FaUsers color='oklch(0.551 0.027 264.364)' /> Author Performance
              </ChartTitle>
            </ChartHeader>
            <ReusableTable 
      ...
      {activeTab === 'subscribers' && (
        <ChartContainer>
          <ChartHeader>
            <ChartTitle>
              <FaUsers color='oklch(0.551 0.027 264.364)' /> Subscriber Growth (6 Months)
            </ChartTitle>
          </ChartHeader>
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart data={subscriberGrowthData}>
              ...
              <YAxis />
              <Tooltip />
              <Area type="monotone" dataKey="subscribers" stroke="#10b981" fillOpacity={1} fill="url(#subscriberGradientLarge)" />
            </AreaChart>
          </ResponsiveContainer>
        </ChartContainer>
      )}
