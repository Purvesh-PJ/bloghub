...
describe('GET /api/posts?topic=', () => {
  it('queries the whole collection, not just the page the client already had', async () => {
    ...
    await Post.updateMany({ _id: { $in: ids } }, { $set: { tags: [topic._id] } });

    // Noise in another topic, published after, so it would dominate a newest-first page.
    for (let i = 0; i < 10; i += 1) await makePost(token, `Unrelated ${i}`);

    const response = await request(app).get('/api/posts?topic=widgets&limit=20');
  ...
  it('returns nothing for a topic that does not exist', async () => {
    const token = await signIn();
    await makePost(token, 'Something');

    const response = await request(app).get('/api/posts?topic=nosuchtopic');
...
describe('GET /api/tags', () => {
  it('returns tags and reports post counts', async () => {
    const token = await signIn();
    const Tag = require('../models/tag.model');
    const [used, unused] = await Tag.create([
      { name: 'usedtag', posts: [] },
      { name: 'unusedtag', posts: [] },
    ]);
    expect(unused.name).toBe('unusedtag');

    const id = await makePost(token, 'In a used tag story');
    await Post.updateOne({ _id: id }, { $set: { tags: [used._id] } });

    const response = await request(app).get('/api/tags');
    const names = response.body.data.map((c) => c.name);

    expect(names).toContain('usedtag');
  });
});
