...
exports.updateCategoryCollection = async (req, res) => {
    ...
    try {
        ...
        const finalCategoriesToAdd = categoryIdsToAdd.filter(catId => !post.categories.some(postCat => postCat._id.toString() === catId));
        const finalCategoriesToRemove = categoryIdsToRemove.filter(catId => post.categories.some(postCat => postCat._id.toString() === catId));
        ...
        if(finalCategoriesToAdd.length > 0 || finalCategoriesToRemove.length > 0){
            if(finalCategoriesToRemove.length > 0){
                ...
                post.categories = post.categories.filter(cat => !finalCategoriesToRemove.includes(cat._id.toString()));
            }
